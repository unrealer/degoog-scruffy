...
// Self-contained: degoog copies each engine directory independently on
// install, so engines cannot share a cross-directory module.
// Request/response shapes mirror Kagi's official OpenAPI types:
...
export default class KagiImagesEngine {
  ...
  settingsSchema = [
    ...
    },
    {
      ...
      type: "number",
      default: 20,
      description: "Maximum image results requested per page (Kagi allows 1-1024).",
    },
    {
      key: "safeSearch",
      label: "Safe Search",
  ...
  async executeSearch(query, page = 1, _timeFilter, context) {
    const data = await kagiSearch(
      { apiKey: this.apiKey, limit: this.limit, safeSearch: this.safeSearch, workflow: "images", name: this.name },
      query,
      page,
      context,
    );
    ...
    return items
      .filter((item) => item && item.url && item.image?.url)
      .map((item) => ({
        title: _clean(item.title) || _host(item.url),
        url: item.url,
        snippet: _host(item.url),
        source: this.name,
