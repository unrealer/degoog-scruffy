// Kagi Images engine for degoog (workflow: images).
...
const _host = (u) => { try { return new URL(u).hostname.replace(/^www\./, ""); } catch { return ""; } };
...
export default class KagiImagesEngine {
  isClientExposed = false;
  name = "Kagi Images";
  bangShortcut = "kagiimages";
  settingsSchema = [
    ...
    { key: "limit", label: "Results per page", type: "number", default: 20, description: "Maximum image results requested per page (Kagi allows 1-1024)." },
  ...
  configure(settings) {
    ...
    this.limit = Number.isFinite(parsed) && parsed > 0 ? Math.min(parsed, 1024) : 20;
  ...
  async executeSearch(query, page = 1, _timeFilter, context) {
    const data = await kagiSearch({ apiKey: this.apiKey, limit: this.limit, safeSearch: this.safeSearch, workflow: "images", name: this.name }, query, page, context);
    const items = data?.image;
    if (!Array.isArray(items)) return [];
    return items.filter((item) => item && item.url && item.image?.url).map((item) => ({ title: _clean(item.title) || _host(item.url), url: item.url, snippet: _host(item.url), source: this.name, thumbnail: item.image.url, imageUrl: item.image.url }));
  }
}
