// Kagi Podcasts engine for degoog (workflow: podcasts).
// degoog has no dedicated podcasts result category, so this renders as web.
...
export default class KagiPodcastsEngine {
  isClientExposed = false;
  name = "Kagi Podcasts";
  bangShortcut = "kagipodcasts";
  settingsSchema = [
    ...
    { key: "limit", label: "Results per page", type: "number", default: 10, description: "Maximum podcast results requested per page (Kagi allows 1-1024)." },
  ...
  async executeSearch(query, page = 1, _timeFilter, context) {
    const data = await kagiSearch({ apiKey: this.apiKey, limit: this.limit, safeSearch: this.safeSearch, workflow: "podcasts", name: this.name }, query, page, context);
    const items = data?.podcast;
    if (!Array.isArray(items)) return [];
    return items.filter((item) => item && item.url && item.title).map((item) => ({ title: _clean(item.title), url: item.url, snippet: _clean(item.snippet), source: this.name, ...(item.image?.url ? { thumbnail: item.image.url } : {}) }));
  }
}
