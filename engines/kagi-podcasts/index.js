...
// degoog has no dedicated "podcasts" result category, so this renders as a
// normal (web) engine — best used as a bang: !kagipodcasts <query>.
...
export default class KagiPodcastsEngine {
  ...
  settingsSchema = [
    ...
    },
    {
      ...
      type: "number",
      default: 10,
      description: "Maximum podcast results requested per page (Kagi allows 1-1024).",
    },
    {
      key: "safeSearch",
      label: "Safe Search",
  ...
  async executeSearch(query, page = 1, _timeFilter, context) {
    const data = await kagiSearch(
      { apiKey: this.apiKey, limit: this.limit, safeSearch: this.safeSearch, workflow: "podcasts", name: this.name },
      query,
      page,
      context,
    );
